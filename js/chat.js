...
class Auth {
    ...
    auth;
    usernameBox;
    constructor(user, handler, auth, usernameBox){
        this.user = user;
        this.auth = auth;
...
class OnlineList {
    ...
    onlineUsers;
    constructor(){
        this.onlineList = new Elem("div", [
            new Attribute("class", "onlineList")
        ], "Online users:").toHTML();
        this.onlineUsers = new Elem("ul", [
            new Attribute("id", "onlineUsers")
        ], "").toHTML();
        this.onlineList.append(this.onlineUsers);
    }
    add(user) {
        this.onlineUsers.append(new Elem("li", [
            new Attribute("id", user)
        ], user).toHTML());
    }
    remove(user) {
...
class Elem {
    ...
    attributes;
    str;
    constructor(tag, attributes, str){
        this.tag = tag;
...
receiveHandlers.set("bcast", new HtmlModifier(chat.chat, (html, msg)=>{
    html.appendChild(new Elem("div", [
        new Attribute("class", "chatMessage")
    ], msg.body).toHTML());
}));
receiveHandlers.set("date", new HtmlModifier(chat.chat, (html, msg)=>{
    html.appendChild(new Elem("div", [
        new Attribute("class", "chatMessage")
    ], msg.body).toHTML());
}));
receiveHandlers.set("dm", new HtmlModifier(chat.chat, (html, msg)=>{
    html.appendChild(new Elem("div", [
        new Attribute("class", "chatMessage")
    ], msg.body).toHTML());
}));
receiveHandlers.set("error", new HtmlModifier(null, (html, msg)=>{
    console.log(msg.toSockData().str);
}));
receiveHandlers.set("online", new HtmlModifier(onlineList.onlineUsers, (html, msg)=>{
    for (const user of msg.body.split('\n').filter(Boolean)){
        html.appendChild(new Elem("li", [
            new Attribute("id", user)
        ], user).toHTML());
    }
}));
...
sendHandlers.set("bcast", new HtmlModifier(null, (html, msg)=>{
    socket.send(msg.toSockData().str);
}));
sendHandlers.set("date", new HtmlModifier(null, (html, msg)=>{
    socket.send(msg.toSockData().str);
}));
sendHandlers.set("dm", new HtmlModifier(null, (html, msg)=>{
    socket.send(msg.toSockData().str);
}));
sendHandlers.set("online", new HtmlModifier(null, (html, msg)=>{}));
const sendHandler = new Handler(sendHandlers);
const chatInput = new ChatInput((instance)=>{
...
const usernameBoxElem = document.getElementById("usernameBox");
if (authElem === null || usernameBoxElem === null) {
    throw new Error("auth or usernameBox element id not found");
}
const auth = new Auth(new User(), (instance)=>{
